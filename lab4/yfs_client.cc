...
int
yfs_client::delEntry(inum dir, inum &ino, std::string &name)
{
  int r = OK;
  //check dir
  extent_protocol::attr a;
  ...
  EXT_RPC(readdir_unlock(dir, all_files));
  //lc->acquire(dir);
  for(it = all_files.begin(); it != all_files.end(); it++){
    if(file_name == it->name)
      break;
  ...
  EXT_RPC(ec->put(dir, ost.str())); //write back
  //lc->release(dir);
...
int
yfs_client::addEntry(inum dir, inum ino, std::string &name)
{
  int r = OK;
  //check dir
  extent_protocol::attr a;
  ...
  bool found;
  inum temp_ino;
  lookup_unlock(dir, name.c_str(), found, temp_ino);
  if(found)
    return false;
