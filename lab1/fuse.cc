        ...
        off_t off, struct fuse_file_info *fi)
{
    ...
    if ((r = yfs->read(ino, size, off, buf)) == yfs_client::OK) {
        //std::cout<< "fuse:fuseserver_read "<<"size "<<buf.size()<<"data "<<buf<<'\n';
        ...
        struct fuse_file_info *fi)
{
    ...
    if ((r = yfs->write(ino, size, off, buf, size)) == yfs_client::OK) {
        //std::cout<<"fuse: fuseserver_write"<<"size:"<<size<<"data "<<buf<<'\n';
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
  ...
  if((r == yfs->readlink(ino, temp_link)) == yfs_client::OK){
    fuse_reply_readlink(req, temp_link.c_str());
  }else{
    fuse_reply_err(req, r);
  }
}
